...
public class LoginStuController {
	...
	public String LoginStuDao(HttpSession session, int all_cd, Login_All_Dto alldto,
		...
		Login_All_Dto alldto_stu = lss.LoginStu(map);
		Login_All_Dto alldto_pro = lss.LoginPro(map);
		Login_All_Dto alldto_sta = lss.LoginSta(map);
		
		if (alldto_stu != null || alldto_pro != null || alldto_sta != null) {
			if (all_cd < 2020201 && all_cd > 2020100) {
				session.setAttribute("login", alldto_stu);
				return "layout1";
			} // 학생 번호 2020101 ~ 2020200까지 로그인 시 학생 페이지로 이동
			if (all_cd < 2020300 && all_cd > 2020200) {
				session.setAttribute("login", alldto_pro);
				return "layout2";
			} // 교수 번호 2020201 ~ 2020300까지 로그인 시 교수 페이지로 이동
			if (all_cd < 2020401 && all_cd > 2020300) {
				session.setAttribute("login", alldto_sta);	
				return "layout3";
			} // 교직원 번호 2020301 ~ 2020400까지 로그인 시 교직원 페이지로 이동
			if (all_cd == 2020300) {
				session.setAttribute("login", alldto_sta);
				return "layout4";
			} // 관리자 번호 2020300으로 로그인 시 관리자 페이지로 이동
		} else {
